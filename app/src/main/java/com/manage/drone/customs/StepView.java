...
public class StepView extends View {
    ...
    private HashMap<String, Boolean> stepMap = new HashMap<>();
    ...
    private void applyStyles(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (descriptions != null) {
            for (CharSequence description : descriptions) {
                steps.add(new Step(description.toString(), false));
            }
    ...
    private void drawEditMode() {
        if (isInEditMode()) {
            if (displayMode == DISPLAY_MODE_WITH_TEXT) {
                if (steps.isEmpty()) {
                    steps.add(new Step("Step 1", false));
                    steps.add(new Step("Step 2", false));
                    steps.add(new Step("Step 3", false));
                }
                setSteps(steps);
            } else {
                if (stepsNumber == 0) {
    ...
    public void setDone(int step) {
        if (step < 0 || step >= steps.size()) return;
        steps.get(step).setDone(true);
    }
    public void reverse(int step) {
        if (step < 0 || step >= steps.size()) return;
        steps.get(step).setDone(false);
    }
    ...
    private int measureStepsHeight() {
        ...
        for (int i = 0; i < steps.size(); i++) {
            String text = steps.get(i).getStep();
    ...
    private void drawStep(Canvas canvas, int step, int circleCenterX, int circleCenterY) {
        final String text = displayMode == DISPLAY_MODE_WITH_TEXT ? steps.get(step).getStep() : "";
        final boolean isSelected = step == currentStep;
        final boolean isDone = steps.get(step).isDone();
    ...
    public class State {
        ...
        public State steps(List<Step> steps) {
            this.steps = steps;
            return this;
        }
