...
public class PointerSpeedometer extends Speedometer {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (withPointer) {
            ...
            canvas.rotate(90 + size, size * .5f, size * .5f);
            canvas.drawCircle(size * .5f, getSpeedometerWidth() * .5f + dpTOpx(8) + getPadding()
        ...
        canvas.drawCircle(size *.5f, size *.5f, getWidthPa()/14f, circlePaint);
        circlePaint.setColor(c);
        canvas.drawCircle(size *.5f, size *.5f, getWidthPa()/22f, circlePaint);
